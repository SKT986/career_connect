...
export default async function MentorsPage() {
  const [mentors, sessions, myMentorProfile, role, mentorPosts, t] = await Promise.all([
    getVerifiedMentors(),
  ...
  return (
    <div className="mx-auto max-w-3xl space-y-10">
      ...
      <section className="space-y-4">
        <h2 className="text-sm font-semibold text-muted-foreground">{t("upcomingAmaSessions")}</h2>
        {sessions.length === 0 ? (
      ...
      <section className="space-y-4">
        <h2 className="text-sm font-semibold text-muted-foreground">{t("verifiedMentors")}</h2>
        {mentors.length === 0 ? (
          <p className="rounded-3xl border border-dashed border-border py-8 text-center text-sm text-muted-foreground">
            {t("noMentorsYet")}
          </p>
      ...
      <section className="space-y-4">
        <div className="flex items-center justify-between gap-4">
          <h2 className="text-sm font-semibold text-muted-foreground">{t("successStories")}</h2>
          <Button asChild variant="ghost" size="sm" className="gap-1 rounded-full text-muted-foreground">
            <Link href="/feed">
              {t("viewInCommunity")} <ArrowRight className="h-3.5 w-3.5" />
            </Link>
          </Button>
        </div>
        {mentorPosts.length === 0 ? (
          <div className="flex flex-col items-center gap-3 rounded-3xl border border-dashed border-border py-12 text-center">
            ...
            <p className="text-sm text-muted-foreground">{t("mentorPostsEmpty")}</p>
          </div>
        ) : (
